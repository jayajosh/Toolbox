...
export function ItemPage({ id, onNavigate }: ItemPageProps) {
  ...
  if (loading) return <main className="app-main">{pageHeading}<p className="state-message">Loading item...</p></main>
  ...
  return (
    <main className="app-main">
      ...
      <div className="form-layout">
       ...
       {checkoutOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setCheckoutOpen(false) }}><section className="bulk-modal" role="dialog" aria-modal="true" aria-labelledby="checkout-title"><div className="bulk-modal-header"><div><p className="kicker">Item checkout</p><h2 id="checkout-title">Who is taking it?</h2></div><button type="button" onClick={() => setCheckoutOpen(false)} aria-label="Close checkout">x</button></div><div className="bulk-modal-body"><label className="field"><span>Borrower</span><input autoFocus value={borrowerName} onChange={(event) => setBorrowerName(event.target.value)} maxLength={200} /></label><label className="field"><span>Notes <small>Optional</small></span><textarea value={checkoutNotes} onChange={(event) => setCheckoutNotes(event.target.value)} rows={3} maxLength={2000} /></label></div><div className="bulk-modal-actions"><button className="secondary-button" type="button" onClick={() => setCheckoutOpen(false)}>Cancel</button><button className="primary-button" type="button" disabled={!borrowerName.trim() || saving} onClick={() => void checkOut()}>{saving ? 'Checking out...' : 'Check out'}</button></div></section></div>}
       {checkinOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setCheckinOpen(false) }}><section className="bulk-modal" role="dialog" aria-modal="true" aria-labelledby="checkin-title"><div className="bulk-modal-header"><div><p className="kicker">Item check in</p><h2 id="checkin-title">Return this item?</h2></div><button type="button" onClick={() => setCheckinOpen(false)} disabled={saving} aria-label="Close check in">x</button></div><div className="bulk-modal-body"><label className="field"><span>Notes <small>Optional</small></span><textarea autoFocus value={checkinNotes} onChange={(event) => setCheckinNotes(event.target.value)} rows={3} maxLength={2000} placeholder="Return condition or detail" /></label></div><div className="bulk-modal-actions"><button className="secondary-button" type="button" onClick={() => setCheckinOpen(false)} disabled={saving}>Cancel</button><button className="primary-button" type="button" disabled={saving} onClick={() => void checkIn()}>{saving ? 'Checking in...' : 'Check in'}</button></div></section></div>}
